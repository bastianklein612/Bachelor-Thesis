...
Given the same inputs, a deterministic simulation will always produce the same results, thus giving the researcher complete reproducibility of any problems which might occur.\todo{put into solution}
...
Our key ideas are threefold: First, we construct a virtual model of a hexapod robot with the help of MATLABs Simulink environment.
Second, to test the created model and to enable the hexapod to walk, we develop a simple locomotion controller capable of performing different gaits and movement speeds.
Third, we use a Reinforcement Learning (RL) approach to learn the coordination rules of the hexapods legs from the ground up.

The majority of work we do for this thesis on the \textit{MATLAB\textsuperscript{\textregistered}} software framework and the MATLAB-based \textit{Simulink\textsuperscript{\textregistered}} environment, both developed by the company \textit{MathWorks\textsuperscript{\textregistered}}.
...
We implement the locomotion controller in Simulink as well, development being divided into three separate steps:
Initially, the attention lies on enabling precise movement of the individual hexapod legs.
Subsequently, we develop a method of generating continuous leg trajectories, focusing in the division between swing and stance cycle.
