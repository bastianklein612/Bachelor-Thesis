...
\todo{For spatial contact force parameters, see \parencite{trotta2022walking}.}
...
\end{align}
The parameter $t$ is limited to the ranges $[0,T_sw]$ and $[0, T_st]$, because if one of the subsystems reaches the end of its phase ($t=T_sw$ or $t=T_st$), the subsystems internal clock is reset and thus upon its next activation, the function evaluation begins from $t=0$ again.
...
We define the MDP tuple $\mathcal{\{S,A,P,R,\gamma\}}$ as follows:
...
The reward function $\mathcal{R}$ given by: $R(t) = s_x(t) + v_x(t) - v_y(t) - \delta_\text{h}(t) - 0.025 \cdot W(t) \ (+ r_\text{survival})$.\\
