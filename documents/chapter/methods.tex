...
The leg trajectory generator is responsible for determining the joint angles required to allow the leg's end-effector to follow a smooth trajectory, all in accordance to $f_c$, $P_\text{swing}$, $s_i\ (i \in \{1,...,6\})$ and $\vec{d}$.
...
$K_p$, $K_i$, $K_d$ (and $N$), the PID parameters, are tuned via trial-and-error method and are given in table \ref{table: PID parameters}.
...
Finalizing the trajectory generator, the analytical IK solver receives the trajectory coordinates and translates them into the \textalpha, \textbeta \ and \textgamma \ joint angles.
A detailed explanation of the solver is provided in \ref{subsubsec: IK Solver}.
...
\begin{figure}[!h]
	\centerline{\includegraphics[scale=0.03]{HexapodLeg_Lengths.PNG}}
...
\begin{table}[h!]
	...
	\begin{tabular}{| c | c |}
		...
		\hline
	\end{tabular}
	\caption[PID parameters]{Gain parameters used for PID-tuning}
