...
To import the .urdf-model we us a MATLAB-internal function to convert the urdf-file into a Simulink model.
The resulting model consists of blocks from the Simscape library, more specifically joints, rigid transforms and coordinate frames to describe the robots kinematic chains and rigid bodies to represent the individual links geometry as well as mass and inertia.
The resulting 3D model of the hexapod can be seen in Fig. \ref{figure: PhantomX 3D model}.
...
To be able to use the hexapod model as a development platform, we modify the raw Simulink import.
We reorganise and encapsulate logical subsystems, specifically the individual legs and thorax and add in- and outputs.
From a top level perspective, the modified system is represented by a single \textit{Hexapod} block, which receives control signals as input and outputs sensory information.

One level lower, the hexapod subsystem consists of 6 leg subsystems and one thorax subsystem, as can be seen in \ref[{figure: Hexapod Model Overview}.
