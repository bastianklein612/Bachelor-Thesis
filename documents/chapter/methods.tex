...
According the internally defined gait pattern and the received data input, the controller computes the torques to be applied to each joint of the hexapod.
The controller subsystem can be described as a feedback control system, as it receives feedback from the process it controls and takes this feedback into account when generating new commands to steer the process.
...
A legs control unit receives the desired frequency of the swing-stance cycle ($f_{c}$), the duty-cycle percentage ($P_{swing}$) and the swing-initiation signal ($s_i,\ i \in \{1,...,6\}$) as coordination inputs.
...
The Simulink RL Agent, which is a separate component of the controller subsystem, receives observations from the environment and hexapod model as well as a reward signal and outputs an action vector which consists of the swing initiation signal for each leg during the current time step.
...
\textbf{Swing and stance phase functions}
\begin{align}
	T_{sw}(t) & = 
	\begin{cases}
	...
	\end{cases}, t \in [0,\frac{P_\text{swing}}{f_c}] 
	\\
	T_{st}(t) & =
	\begin{cases}
		x_{st}(t) = \\
		z_{st}(t) = 0
	\end{cases}, t \in (\frac{P_\text{swing}}{f_c}, \frac{1}{f_c}]    
\end{align}
...
As we are not able to achieve this compute frequency with the provided iterative solver, we implement our own, purpose-build analytical IK solver aiming to drastically improve computation time.
