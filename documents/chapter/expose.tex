...
Compared to wheeled or bipedal architectures, the presence of six legs enables these robots to more easily overcome challenging terrain and complex obstacles \parencite{barai2013smart, atifystructure}.
This feature makes them particularly interesting for navigating landscapes otherwise inaccessible by humans or other robotic systems.

\section{Problem}
The process of developing software for these hexapods is a challenging and by no means trivial task.
While maintaining balance for example, is evidently easier for this architecture than for the above mentioned bipedal robots, coordinating the many legs during movement poses a major challenge \parencite{azayev2020blind,schilling2013walknet}.
...
\section{Objective}
In this thesis, we aim to develop a comprehensive virtual model of a hexapod robot.
The framework we are developing serves as a foundation for our future research to build on.
To facilitate flexible and easy expansions, the model requires a high level of modularity, allowing for the integration and replacement of individual subsystems without having to reconfigure the whole system.
The simulation environment supports accurate physics simulations, including collision detection, friction modeling and dynamic interactions to realistically simulate the robots movements.
Additionally, it provides accessible means to observe and analyze the robots behavior.
This includes the ability to extract data from the simulation and visualize it, providing better evaluation of the systems performance.
Furthermore, the surrounding software framework enables the future use of tools from various domains, such as control/ learning algorithm development or hardware integration.
...
The basis of the work is provided by the software \textit{MATLAB\textregistered}, which is widely used in research and industry, and the included \textit{Simulink}\textregistered environment of the company \textit{MathWorks}.
