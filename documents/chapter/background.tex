...
By emulating the biomechanics and behavior of insects, researchers and engineers aim to create versatile and robust robotic systems capable of navigating challenging terrain \parencite{irawan2011optimal, ouyang2021adaptive, schilling2013walknet}.
...
Their behavior can be discrete, like a switchor logic gate which activate when specific signals are pulled high, or represent more complex and continuous functions such as integrators, derivatives or sine waves.
...
\begin{figure}[h]
	\centerline{\includegraphics[scale=0.5]{morphologyOfStickInsect_sketch}}
...
There are two distinct methods how to calculate these angles, analytical and numerical \parencite{inverseKinematicsIllinois}.
...
These kinds of systems potentially lack closed-form expressions for the solutions, which can make deriving analytical solutions infeasible if not impossible.
...
Due to their iterative nature numerical solvers are generally more computationally expensive, given the same problem, than their analytical counterparts \parencite{aristidou2018inverse, inverseKinematicsIllinois}.
...
A \textbf{P}roportional-\textbf{I}ntegral-\textbf{D}erivative(PID) controller is a common feedback control system \parencite{aastrom2002control}.
...
A PID controller uses three terms, a proportional, integral and derivative term, to compute the controlled output based on the error between the setpoint and current process variable.
...
The basic underlying principle of RL is to learn how a scalar reward signal can be cumulatively maximized \parencite{sutton2018reinforcement}.
In simple terms, RL  is about taking appropriate measures to maximize a reward signal in a particular situation.
focuses on training an agent(or multiple agents) to take sequences of actions in an environment as to maximize a cumulative reward.
...
\begin{figure}[h]
	\centerline{\includegraphics[scale=0.1]{RL_Overview}}
...
Every problem for which as a reward function can be defined which accurately represents the agents performance reinforcement learning can be applied to \parencite{silver2015}.\todo{refactor this section}
The agent can only influence the environment by taking actions.
...
In reinforcement learning, there are 2 distinct types of environments, which differ in the definition of their action space.
